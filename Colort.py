...
class Player(Unit):
    ...
    def draw(self, surface):
        ...
        # Волосы-повязка в стиле ниндзяго
...
def run_game(difficulty):
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("Killer Time")
    clock = pygame.time.Clock()

    settings = DIFFICULTY_SETTINGS[difficulty]

    player = Player(WIDTH // 2, HEIGHT // 2, GREEN, hp=200, speed=settings["player_speed"])
    enemies = []
    apples = []
